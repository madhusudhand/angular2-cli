...
class TsCompile {
  constructor(options) {
    ...
    this.env = {
      path     : path.resolve('./src/environment.ts'),
      basePath : path.resolve('./config'),
    };
  }

  compile(src, dest, env) {
    const _this = this;
    this.env.content = this.getEnvFileContent(env);
  ...
  getEnvFileContent(env) {
    return fs.readFileSync(`./config/environment.${env}.ts`);
  }
